...
## 当前进度
- **阶段**: P6 用户反馈迭代进行中
- **已完成**: P0~P4 全部、UI 调整阶段一~五、P5.0 海报/封面填充、P6.1 体验修复、P6.8 演出详情页优化、P6.2 访谈详情页重做、P6.3 瀑布流布局改版、P6.4 留言板增强、P6.5 相册模块、P6.6 前台编辑模式、P6.10 全站时间排序、P6.11 网格布局统一、P6.12 霖言霖语独立模块、P6.13 详情页图片完整显示、P6.15 相册瀑布流布局、P6.16 活动封面+媒体上传、P6.17 Markdown富文本+编辑自动刷新、P6.19 霖言霖语文字-媒体联动、P6.20 霖言霖语详情页UI精细化、P6.21 舞台详情页改造+综艺关联、P6.22 影视详情页媒体上传、P6.23 留言板卡片布局优化、P6.24 霖言霖语封面上传+卡片图片展示、P6.25 移动端编辑模式按钮
- **下一步**: P6.7 → P6.9
...
> **绝对禁止运行 `prisma db seed` 或任何会 deleteMany / 清空表的脚本！**
>
> 生产数据库中已有大量手动录入的数据（图片、关联关系等），seed 脚本会先清空所有表再重建，**导致不可逆的数据丢失**。
>
> **正确做法：**
> - 插入样本数据 → 使用 `prisma.xxx.upsert()` 或 `prisma.xxx.create()` 单条操作
> - Schema 变更 → 使用 `prisma db push` 或 `prisma migrate dev`（只改结构，不动数据）
> - 需要批量写入 → 编写独立脚本，**只 create/upsert，绝不 deleteMany**
>
> **`prisma/seed.ts` 仅作为初始化参考，禁止在已有数据的环境中执行。**
