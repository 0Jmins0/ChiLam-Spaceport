...
| 阶段 | 内容 | 预估工作量 | 依赖 |
|------|------|-----------|------|
| P6.1 | 体验修复（图片优化 + 返回导航 + 留言即时更新） | 小 | 无 |
| P6.2 | 访谈详情页重做（三栏布局 + 音视频播放器） | 中 | Schema 变更 |
| P6.3 | 瀑布流布局改版（全站列表页 CSS columns） | 中 | 无 |
| P6.4 | 留言板增强（精选权重 + 图片上传） | 中 | Schema 变更 |
| P6.5 | 相册模块（新栏目 `/gallery`，留言板左侧） | 大 | Schema 变更 |
| P6.6 | 前台编辑模式（管理员内联编辑系统） | 大 | 无 |
| P6.7 | 全局隐藏描述字段（搜索增强） | 中 | Schema 变更 |
| P6.8 | 演出详情页优化（新增简介 + 布局调整） | 小 | Schema 变更 |
| P6.9 | 用户收藏功能（收藏 Media/留言/内容） | 中 | Schema 变更 |
| 持续 | 数据填充（新闻爬取、舞台整理、关联绑定） | 持续 | P5.1 继续 |
